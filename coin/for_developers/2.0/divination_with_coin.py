...
class Main():
    def __init__(self):
        ...
        # 构建数据库文件的完整路径
        db_path = os.path.join(script_dir, 'I_Ching.db')  # 请替换为你的实际数据库文件名
        #连接到数据库
    ...
    def input_imp(self):
        ...
        script_dir = os.path.dirname(os.path.abspath(__file__))
         ...
         # 构建图片文件的完整路径
        back_img_path = os.path.join(script_dir, '0.png')
        front_img_path = os.path.join(script_dir, '1.png')

        # 使用完整路径打开图片
        self.back_img = Image.open(back_img_path)
        self.front_img = Image.open(front_img_path)
    ...
    def link_TEXT(self, gua, num):
        search_code = f"{gua}-{num}"
        # 连接数据库利用search_code定位行返回列名为辞和链接的字典
        self.cursor.execute(f"SELECT 辞, 链接 FROM IChing WHERE 查阅码 = '{search_code}'")
        result = self.cursor.fetchone()
        text1=re.search(r'[\u4e00-\u9fa5]$', gua).group()
        text2, link =result[0], result[1]
